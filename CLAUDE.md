...
- Golden-dataset eval: `uv run python -m scripts.eval_golden` checks decomposition against `backend/fixtures/golden_questions.json` (add `--graph` for live retrieval incl. `expect_loops` catalogue ids, `--answers` for code checks on the raw answer). Every run appends a score line to `backend/eval_runs.jsonl`. Extend the dataset when adding intent fields or templates.
