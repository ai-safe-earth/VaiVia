...
- Neo4j: `docker compose --env-file .env -f infra/docker-compose.yml up -d neo4j` (run from repo root; `--env-file` is required because the compose file lives in `infra/`) (Community + APOC + GDS — GDS is only for `scripts.check_graph_connectivity` now). Copy `.env.example` → `.env` first; `NEO4J_PASSWORD` is required (no default).
...
- Golden-dataset eval: `uv run python -m scripts.eval_golden` checks decomposition against `backend/fixtures/golden_questions.json` (add `--graph` for live retrieval incl. `expect_trails` ids and `expect_facts` bands on drawn outings, `--answers` for code checks on the raw answer). Every FULL run appends a score line to `backend/eval_runs.jsonl` — the aggregates plus which entries failed and how each retrieval expectation landed; `--only <ids>` reruns single entries and is deliberately not logged. Extend the dataset when adding intent fields or templates.
...
- Migrations are `pipeline/sql/v2/NNNN_*.sql`, applied in filename order by `uv run python migrate.py` (idempotent; `--dry-run` lists them). Add a new file; never edit an applied one. Schemas: `staging`, `source_map` (network, places), `catalogue` (routes — retired as a product by R7, kept as the pack engine's parity oracle), `qa`, `provenance` (build ledger).
