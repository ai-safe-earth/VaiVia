...
EDGE_DETAILS = [
    {"i": 0, "osm_way_id": "1#0", "surface": "gravel", "gain_m": 120.0},
    {"i": 1, "osm_way_id": "2#0", "surface": "dirt", "gain_m": 80.0},
]
...
def test_gds_route_is_preferred_and_enriched(client, db):
    ...
    assert response.status_code == 200
    body = response.json()
    assert body["total_distance_m"] == 7800.0  # Dijkstra's, not shortestPath's
...
def test_gds_route_over_cap_falls_back_then_404s(client, db):
    _gds_available(db)
    db.when("route_gds_dijkstra", [dict(GDS_ROW, total_m=999_999.0)])
