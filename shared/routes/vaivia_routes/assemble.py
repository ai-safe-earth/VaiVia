...
class WalkedEdge(NamedTuple):
    ...
    # Vertex indices for the connectivity invariant; None when the caller
    # cannot supply them (the pgRouting path predates them).
...
def assert_connected(edges: list[WalkedEdge]) -> None:
    """Consecutive walked end = next walked start, or the route has a hole.

    The planner invariant that replaces the QGIS review for a route a user
    sees seconds after asking (docs/route-design.md, decision 1). Raises
