...
## Project notes (VaiVia)

- The roadmap/plan doc is `docs/plan.md` (phases and checkboxes); session state is
  `handoff.md` (read once per session per CLAUDE.md — don't re-read for this skill).
  Pipeline topics also have `pipeline/docs/metadata-rules.md`.
- Phase names in status tables use `docs/plan.md`'s numbering (Phase 1–7).
- Commits follow Conventional Commits on the current feature branch; PRs target
  `develop`, never `main`.
