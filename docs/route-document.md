...
| `recommended` | of a direction pair, whether THIS direction is the one to suggest | 2.1 (2026-08-27, owner-ratified): the steep side climbed, the gentle side descended — decided by the profile's mean climbing gradient per direction; null when undirected, unpaired, or profileless. First emitted for single-line circular mapped routes, whose walk `export/orientation.py` infers |
