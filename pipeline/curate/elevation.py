...
Elevation describes ONE build of the network, like source_map.edge_route: the
...
PROFILES = "SELECT edge_id, profile_m FROM source_map.edge WHERE profile_m IS NOT NULL"
...
# forgot to clear.
ALIGNMENT = """
SELECT count(*) FROM source_map.edge
...
def check(conn) -> int:
    ...
    row = conn.execute(
        "SELECT count(*) FILTER (WHERE profile_m IS NOT NULL), count(*) FROM source_map.edge"
    ).fetchone()
    profiled, edges = row
    if profiled == 0:
...
def main() -> None:
    ...
    with connect() as conn:
        ...
        (edges,) = conn.execute("SELECT count(*) FROM source_map.edge").fetchone()
        if not edges:
        ...
        uncovered = conn.execute("""
            SELECT count(*) FROM source_map.vertex v
        ...
        conn.execute("UPDATE source_map.edge SET ascent_m = NULL, descent_m = NULL")
        with conn.cursor() as cur:
            ...
            with cur.copy(
                ...
                for row in climbs:
                    copy.write_row(row)
            cur.execute(
                "UPDATE source_map.edge e SET ascent_m = c.ascent_m, descent_m = c.descent_m"
        ...
        (vertices,) = conn.execute(
            "SELECT count(*) FROM source_map.vertex WHERE elevation_m IS NOT NULL"
        ).fetchone()
        lo, hi = conn.execute(
            "SELECT min(elevation_m), max(elevation_m) FROM source_map.vertex"
        ).fetchone()
        summary |= {
